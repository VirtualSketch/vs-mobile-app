...
extern "C" const char *predictWithPath(const char *imagePath)
{
    ...
    // output += "First three bytes of Uint8List: " + to_string(bytelist[0]) + " " + to_string(bytelist[1]) + " " + to_string(bytelist[2]);

    // output += "\nImage rows: " + to_string(image.rows) + " and " + "image cols: " + to_string(image.cols);
